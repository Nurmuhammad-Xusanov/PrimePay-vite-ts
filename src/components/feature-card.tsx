...
const FeatureCard: FC <Props> = ({ icon, title, content, idx }) => {
	return (
        <div className={`flex  flex-row p-6 rounded-[20px] feature-card cursor-pointer ${idx === features.length - 1 ? 'mb-0' : 'mb-6'}`}>
                <div className={`w-[64px] h-[64px] rounded-full bg-lightBlue ${styles.flexCenter}`}> {icon} </div>
